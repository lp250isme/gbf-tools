...
A Tampermonkey/Violentmonkey/Greasemonkey userscript (`core/gbf-tools.user.js`) for the web version of Granblue Fantasy (碧藍幻想). It injects CSS and JS to enhance the game's browser UI. Forked from biuuu's original gist.

## Architecture

The entire codebase is a single IIFE in `core/gbf-tools.user.js`:
...
No build system, no dependencies. Edit `core/gbf-tools.user.js` directly. Test by pasting into a userscript manager and loading the game.
