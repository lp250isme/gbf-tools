...
還支援：
- **浮動可拖**：整條浮在遊戲上方，抓握把 **⠿** 拖到任一空白處、放開就記住位置（本機，不同步——各裝置螢幕尺寸不同），不會再蓋到遊戲自己的按鈕；拖出畫面也會自動夾回來。版面分兩排：上排控制（握把／⚙／分類鈕，透明無底），下排捷徑（黑底藥丸）。
- **分類**：每個捷徑可填「群組」。有多個群組時，上排會出現一顆 **`▸ 分類名`** 輪替鈕，**按一下換下一個分類**，下排只顯示該分類的捷徑（只有一個群組時不顯示輪替鈕）。
