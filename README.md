...
用 [DeepL](https://www.deepl.com/pro-api)（免費方案每月 50 萬字元，遊戲短文字綽綽有餘）。安裝方式同上（點 `gbf-translate.user.js` 的「Raw」）。裝好後：

1. 到 [DeepL API](https://www.deepl.com/pro-api) 免費註冊，拿到 API key（Free 方案的 key 以 `:fx` 結尾，腳本會自動走免費端點）。
2. 在遊戲頁面點瀏覽器右上角 **Tampermonkey 圖示 → 「🔑 設定 DeepL API Key」**，貼上 key。
3. 完成，技能/武器說明會自動變中文。
...
| 🔑 設定 DeepL API Key | 填 / 改 key |
| 🈯 切換 繁中 / 簡中 | 目標語言（繁/簡各自獨立快取） |
| 🗑 清除翻譯快取 | 清掉本機已快取的譯文 |

> ⚠ key 只填在你本機的腳本管理器裡，**勿**提交回 repo。標頭 `@connect` 已鎖定只連 DeepL 端點。
